...
The local launcher exposes the complete `D:\UW` tree through the Document Register. It indexes the full filesystem manifest, while preserving structured accounting records separately; PDFs and image files can be viewed in-app, and Office documents can be opened or downloaded from the register. When the app is downloaded to another machine, place the source files in a `documents` or `newstatements` folder beside the app, or set `UW_SOURCE_DIR` to the real source folder before starting `server.js`; the viewer resolves both historical absolute paths and filename-only statement records against those folders. Always start the local server (do not open `index.html` directly) so the source-file route is available. Existing invoices and quotes include Edit actions. A quote converted to an invoice keeps a source link and can be undone while no payment or receipt has been recorded; this returns the quote to Draft without deleting financial history from paid invoices. Jobs can be linked to invoices, and linked totals/balances are read live from the invoice ledger. Backup/copy and lock artifacts remain visible for audit but are flagged and are not included in primary imported totals.
